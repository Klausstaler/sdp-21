...
class Node:
    ...
    def get_junction_number(self): # gets the respective value for the node texture depending on its connection and type
        ...
        elif self.type == "RFID" or "Robot":

            if self.up != -1 and self.right == -1 and self.down != -1 and self.left == -1: # vertical
                return 5
            if self.up == -1 and self.right != -1 and self.down == -1 and self.left != -1: # horizontal
                return 4
...
grid = get_grid(p.dimensions, p.nodes)
