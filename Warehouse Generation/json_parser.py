...
class Node:
    ...
    def get_junction_number(
            self):  # gets the respective value for the node texture depending on its connection and type
        # print(self.type)
        if self.type == "shelf":
            if self.up != -1:
                return 0
        ...
        #     return 15
            ...
            if (self.up != -1 and self.right == -1 and self.down != -1 and self.left == -1) or (
                    self.right == -1 and self.left == -1 and (self.up != -1 or self.down != -1)):  # vertical
                return 5
            if (self.up == -1 and self.right != -1 and self.down == -1 and self.left != -1) or (
                    self.up == -1 and self.down == -1 and (self.right != -1 or self.left != -1)):  # horizontal
                return 4
...
def get_grid(dimensions, nodes):  # creates a grid representation of a graph for texture/map generation.
    ...
    grid_ids = -np.ones([width, height], dtype=np.int32)
